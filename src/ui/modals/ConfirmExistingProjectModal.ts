  ...
  onOpen() {
    ...
    new Setting(contentEl).addButton((btn) =>
      btn.setButtonText("Open Existing").onClick(() => {
        debug(`${DEBUG_PREFIX} ConfirmExistingProjectModal: opening existing project "${this.path}"`);
        this.close();
        this.onOpenExisting();
      }),
    );
    ...
    new Setting(contentEl).addButton((btn) =>
      btn
        ...
        .setCta()
        .onClick(() => {
          debug(`${DEBUG_PREFIX} ConfirmExistingProjectModal: creating new project anyway "${this.path}"`);
          this.close();
